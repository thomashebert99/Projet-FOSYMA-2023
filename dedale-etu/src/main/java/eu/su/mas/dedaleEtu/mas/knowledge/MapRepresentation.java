...
public class MapRepresentation implements Serializable {
	...
	public String centralize(){
        BetweennessCentrality bcb = new BetweennessCentrality();
        bcb.init(this.g);
        bcb.compute();
        double maxCb = 0;
        Node node = this.g.getNode(0);
        for(String id : this.getAllNodes()){
            double tmp =  (double) this.g.getNode(id).getAttribute("Cb");
            if(maxCb < tmp){
                maxCb = tmp;
                node = this.g.getNode(id);
            }
        }
        return node.toString();
    }
	
	public List<String> getNeighborNodes(String id) {
		List<String> result = new ArrayList<String>();
		Node x = g.getNode(id);
		Iterator<Node> ite = x.neighborNodes().iterator();
		while (ite.hasNext()) {
			Node y = ite.next();
			result.add((String) y.getAttribute("ui.label"));
		}
		return result;
	}
